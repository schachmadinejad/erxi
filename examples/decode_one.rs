use erxi::decoder::{decode_with_schema, Decoder};
...
fn main() {
    let path = std::env::args().nth(1).expect("usage: decode_one <exi>");
    let path = Path::new(&path);
    let data = std::fs::read(path).expect("read");
    let options = options_from_filename(path);
    let schema = schema_for_fixture(path).expect("schema");
    let no_schema = std::env::var("ERXI_NO_SCHEMA").is_ok();
    let use_decode_with_schema = std::env::var("ERXI_USE_DECODE_WITH_SCHEMA").is_ok();
    if no_schema {
        match erxi::decoder::decode_with_options(&data, options) {
            Ok((events, _)) => {
    ...
    if options.compression() || use_decode_with_schema {
        match decode_with_schema(&data, options, schema) {
            Ok((events, _)) => {
                for (i, event) in events.iter().enumerate() {
                    println!("{:02}: {:?}", i, event);
                }
    ...
    let mut events = 0u32;
    let max_events = 200_000u32;
    loop {
        if events >= max_events {
            println!("ERR event limit exceeded");
            return;
        }
